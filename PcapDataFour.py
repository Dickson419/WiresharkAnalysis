...
def network_conversation(packet):
    try:
        ...
        # Look up the hostname of the source IP address
        try:
            source_hostname = socket.gethostbyaddr(source_address)[0]
        except:
            source_hostname = "Unknown"
        # Look up the hostname of the destination IP address
        try:
            destination_hostname = socket.gethostbyaddr(destination_address)[0]
        except:
            destination_hostname = "Unknown"
        # Return a tuple of the source hostname, source IP address, destination hostname, destination IP address, source port, destination port, and protocol name
        return (source_hostname, source_address, destination_hostname, destination_address, source_port, destination_port, protocol)
